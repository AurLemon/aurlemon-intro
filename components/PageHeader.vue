<template>
	<header class="aurle-page-header">
		<div
			class="aurle-page-header-list"
			ref="headerList"
			@click="handleHeaderClick"
		>
			<div
				class="aurle-page-header-pointer"
				:style="{
					background: `radial-gradient(100px circle at ${pointerX}px ${pointerY}px, var(--background-color-primary--active) 0%, transparent 65%)`,
				}"
			></div>
...
const scrollbarAnimation = () => {
	...
	const mappedMax = 10
	const mappedBounceY =
		((Math.abs(progressDiff) + 100) * (mappedMax - mappedMin)) / 200 + mappedMin // 线性变换
