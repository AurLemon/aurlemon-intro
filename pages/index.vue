<template>
	<div class="aurle-home aurle-page">
		...
		<div
			class="aurle-page-background"
			:style="{ opacity: 1 - scrollProgress / 100 }"
		>
			<div
				class="circle circle-top-left"
				:style="{ transform: `translateY(-${backgroundOffsetValue}px)` }"
			></div>
			<div
				class="circle circle-top-right"
				:style="{ transform: `translateY(-${backgroundOffsetValue}px)` }"
			></div>
			<div
				class="circle circle-bottom-left"
				:style="{ transform: `translateY(-${backgroundOffsetValue}px)` }"
			></div>
			<div
...
useHead({
	meta: [{ name: 'description', content: 'AurLemon 的介绍网站！' }],
})
</script>
...
.basic-intro-enter-active,
.basic-intro-leave-active {
	transition:
		transform 0.5s ease,
		opacity 0.5s ease;
}
...
.home-cards-enter-active,
.home-cards-leave-active {
	transition:
